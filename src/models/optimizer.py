...
def optimize(df_source: pd.DataFrame):
    ...
    for var in paid_vars_imp:
                    ...
                    for percentiles_border in percentiles_border_combs:
                        ...
                        df['competitors_list_tv'] = Carryover(strength=0.8, length=12) \
                            .fit(np.array(df['competitors_list_tv']) \
                                 .reshape(-1, 1)) \
                            .transform(np.array(df['competitors_list_tv']).reshape(-1, 1))
                        ...
                        percentiles = [int(np.percentile(np.array(index), i)) for i in percentiles_border]
                        ...
                        model = sm.ols(formula=formula([f"{var}_trans"] + context_vars), data=df).fit(method="pinv")

                        # display(model.summary())

                        TV_Digital_OOH_Geo = ['reg_tv_trp',
                                              'digital_2020_2022Q1_imp', 'digital_youtube_imp',
                                              'gis_imp', 'full_yandex_maps_imp']
                        ...
                        p_value = np.round(p_values_df[p_values_df['variables'] == f"{var}_trans"]['p'].values[0], 5)
                        ...
                        ans = ans.append(pd.DataFrame([[linear_coef, x_data, y_data, mape, r2, r2_adj, p_value,
                                                        ...
                                                        np.round(roi['roi'].values[0], 2), y_data[0], y_data[1],
        ...
        ans['p_value_norm'] = ((ans['p_value'] / (
                    np.mean(ans['p_value']) + 1e-10) + 1))  # ans['p_value'] / np.mean(ans['p_value'])

        ans['final_metric'] = ans['r2_adj_norm'] / ans['p_value_norm'] / ans['mape_norm'] / ans[
            'wrong_roi'] / ans['wrong_adstock']
